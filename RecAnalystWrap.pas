...
type
  ...
  private
    fGameType: TGameType;
    fGameTypeString: String;
    fMapStyle: TMapStyle;
    fMapStyleString: String;
    ...
    fGameVersionString: String;
    fVictory: String;
    function GetGameTypeString(): String;
    function GetMapStyleString(): String;
    function GetGameVersionString(): String;
  public
    property GameType: TGameType read fGameType write fGameType;
    property GameTypeString: String read GetGameTypeString;
    property MapStyle: TMapStyle read fMapStyle write fMapStyle;
    property MapStyleString: String read GetMapStyleString;
    ...
    property GameVersionString: String read GetGameVersionString;
...
{ TGameSettings }
function TGameSettings.GetGameTypeString(): String;
begin
  if (fGameTypeString = '') then
  begin
    if (fGameType in [Low(GAME_TYPES)..High(GAME_TYPES)]) then
      fGameTypeString := GAME_TYPES[fGameType];
  end;
  Result := fGameTypeString;
end;

function TGameSettings.GetMapStyleString(): String;
begin
  if (fMapStyleString = '') then
  begin
    if (fMapStyle in [Low(MAP_STYLES)..High(MAP_STYLES)]) then
      fMapStyleString := MAP_STYLES[fMapStyle];
  end;
  Result := fMapStyleString;
end;

function TGameSettings.GetGameVersionString(): String;
begin
  if (fGameVersionString = '') then
  begin
    if (fGameVersion in [Low(GAME_VERSIONS)..High(GAME_VERSIONS)]) then
      fGameVersionString := GAME_VERSIONS[fGameVersion];
  end;
  Result := fGameVersionString;
end;
...
begin
  ...
  begin
    fGameType := TGameType(dwGameType);
    fGameTypeString := String(AnsiString(szGameType));
    fMapStyle := TMapStyle(dwMapSize);
    fMapStyleString := String(AnsiString(szMapStyle));
    ...
    fGameVersion := TGameVersion(dwVersion);
    fGameVersionString := String(AnsiString(szVersion));
