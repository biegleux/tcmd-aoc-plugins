...
{$DEFINE SQLITE_DYNLINK} // dynamic linking
...
type
  ...
  private
    ...
    {$IFDEF SQLITE_DYNLINK}
    function GetSQLiteLibLoaded: Boolean;
    {$ENDIF}
  protected
    procedure DoQuery(const value: String);
    function _ExecSQL(const SQL: String): String;
  public
    ...
    procedure ExecSQLs(const SQL: String);
    ...
    procedure BeginTransaction(const name: String = ''; const param: String = '');
    {: Commit transaction.
       If you use non-empty Name parameter, then savepoint is used. Savepoint is named and can be nested.}
    ...
    {$IFDEF SQLITE_DYNLINK}
    property SQLiteLibLoaded: Boolean read GetSQLiteLibLoaded;
    {$ENDIF}
  end;
...
resourcestring
  ...
  {$IFDEF SQLITE_DYNLINK}
  c_libnotloaded = 'SQLite library has not been loaded.';
  {$ENDIF}

{$IFDEF WIN32}
function SystemCollate(Userdata: pointer; Buf1Len: integer; Buf1: pointer;
...
begin
  ...
  {$IFDEF SQLITE_DYNLINK}
  if not SQLiteLibLoaded then
    raise ESqliteException.Create (c_libnotloaded);
  {$ENDIF}
...
var
  ...
  NextSQLStatement: PAnsiChar = nil;
  iStepResult: integer;
begin
  try
...
function TSQLiteDatabase._ExecSQL(const SQL: String): String;
var
  Stmt: TSQLiteStmt = nil;
  NextSQLStatement: PAnsiChar = nil;
  iStepResult: integer;
begin
  try
    ...
    Result := '';
    if (NextSQLStatement <> nil) then
      Result := Trim(String(AnsiString(NextSQLStatement)));
  finally
    if Assigned(Stmt) then
      Sqlite3_Finalize(stmt);
  end;
...
procedure TSQLiteDatabase.ExecSQLs(const SQL: String);
var
  NextSQLStatement: String;
begin
  NextSQLStatement := SQL;
  repeat
    NextSQLStatement := _ExecSQL(NextSQLStatement);
  until (NextSQLStatement = '');
end;
...
procedure TSQLiteDatabase.BeginTransaction(const name: String = ''; const param: String = '');
begin
  Start (name, param);
end;
...
{$IFDEF SQLITE_DYNLINK}
function TSQLiteDatabase.GetSQLiteLibLoaded: Boolean;
begin
  Result := SQLite3d.SQLiteLibLoaded;
end;
{$ENDIF}
